...
@section('title', 'Панель администратора | Основная информация')
...
@section('content')
    <div class="row-12">
        <div class="panel p-3 p-md-5 mt-5">
            ...
            <a href="infos/add" class="btn btn-success panel__btn mb-3">Добавить информацию
                        ...
                        <table class="table table-striped table-bordered">
                            ...
                            <tbody>
                                @foreach ($infos as $info)
                                    <tr data-item-id={{ $info['id'] }}>
                                        <td class="align-middle text-center">
                                            <img class="table__img"
                                                src="/storage/media/infos/{{ $info['id'] . '/' . $info['img'] . '.webp?=r' . rand(0, 999999) }}"
                                                alt="{{ $info['title'] }}">
                                        </td>
                                        <td class="align-middle table__title text-center">{{ $info['title'] }}</td>
                                        <td class="align-middle text-center table__title">
                                            @foreach ($info->city as $city)
                                                {{ $city->name }}
                                            @endforeach
                                        </td>
                                        <td class="align-middle text-center">
                                            <a href="{{ url('admin/infos/edit') }}{{ '/' . $info['id'] }}"
